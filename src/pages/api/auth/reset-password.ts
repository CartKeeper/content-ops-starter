...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
    ...
    const query = await supabaseAdmin
        .from('users')
        .select(
            'id,email,name,roles,role,permissions,password_reset_expires_at,password_reset_token,created_at,updated_at,role_title,phone,welcome_message,avatar_url,status,email_verified_at,calendar_id,workspace_id,deactivated_at,last_login_at'
        )
