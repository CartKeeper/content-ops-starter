...
export function StudioCalendar() {
    ...
    React.useEffect(() => {
        ...
        let isMounted = true;
        const authGetter = (supabase.auth as {
        ...
        authGetter.call(supabase.auth).then(({ data, error }) => {
            if (!isMounted) {
                return;
            }
            if (error || !data?.user) {
    ...
    const { data: clientOptions = [], isLoading: isLoadingClients } = useSWR<ClientOption[]>(
        supabaseConfigured ? 'calendar-client-options' : null,
        fetchClients
    );
    ...
    const {
        ...
        visibleRange && supabaseConfigured ? ['studio-calendar-events', visibleRange.start, visibleRange.end] : null,
        (key) => {
    ...
    const handleSelect = React.useCallback(
        (selection: any) => {
            if (!supabaseConfigured) {
                setToast({
                    id: Date.now(),
                    message: 'Configure Supabase to create events.',
                    variant: 'error'
                });
                return;
            }
        ...
        [currentUserId, openCreateDialog, supabaseConfigured]
    );
    ...
    const handleCreate = React.useCallback(
        async (values: CalendarEventPayload) => {
            if (!supabaseConfigured) {
                throw new Error('Supabase is not configured. Contact an administrator.');
            }
            if (!currentUserId) {
        ...
        [clientOptions, currentUserId, events, mutateEvents, supabaseConfigured]
    );
    ...
    const handleUpdate = React.useCallback(
        ...
        [clientOptions, events, mutateEvents, supabaseConfigured]
    );
    ...
    const handleDelete = React.useCallback(
        ...
        [events, mutateEvents, supabaseConfigured]
    );
    ...
    const updateFromDrag = React.useCallback(
        async (eventApi: any, revert: () => void) => {
            ...
            if (!supabaseConfigured) {
                setToast({
                    id: Date.now(),
                    message: 'Configure Supabase before editing events.',
                    variant: 'error'
                });
                revert();
                return;
        ...
        [canEditEvent, events, handleUpdate, supabaseConfigured]
    );
            ...
            <div className="mb-4 rounded-4 border bg-white p-4 shadow-sm">
                ...
                {!supabaseConfigured ? (
                    <div className="alert alert-warning" role="status">
                        Supabase credentials are not configured. Events will not sync until environment variables are set.
                    </div>
                ) : null}
